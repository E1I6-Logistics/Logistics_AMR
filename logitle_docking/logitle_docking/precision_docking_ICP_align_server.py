...
class PrecisionDockingServer(Node):
    def __init__(self):
        ...
        # TF2 버퍼 및 리스너 초기화 (Nav2 없이도 turtlebot3_node 구동 시 자동 동작)
        ...
        # ---------------------------------------------------------
        # 파라미터 선언 (TurtleBot3 Burger & 도킹 스테이션 실측 반영)
        # ---------------------------------------------------------
        ...
        self.declare_parameter('roi_y_limit', 0.45)
        ...
        # 최종 Yaw 정렬용 파라미터
        self.declare_parameter('final_target_yaw_deg', 0.0)      # 최종 맞출 목표 Yaw (deg)
        ...
        self.get_logger().info('상태 머신 기반 2단계 정밀 도킹 서버 준비 완료.')
    ...
    def get_current_odom_yaw(self) -> float:
        """TF 버퍼에서 odom -> base_link Yaw 각도를 룩업합니다."""
    ...
    def execute_callback(self, goal_handle):
        self.get_logger().info('도킹 시퀀스 시작 (상태 머신 가동)')
        ...
        while rp.ok():
            ...
            # -------------------------------------------------------------
            # [STATE 0: INIT] 데이터 수신 및 타깃 템플릿 생성
            # -------------------------------------------------------------
            if state == DockingState.INIT:
                ...
                self.get_logger().info('>> [상태 전이] INIT -> ICP_APPROACH (1차 도킹 시작)')
                state = DockingState.ICP_APPROACH

            # -------------------------------------------------------------
            # [STATE 1: ICP_APPROACH] 1차 도킹 (ICP 기반 접근 제어)
            # -------------------------------------------------------------
                ...
                if self.latest_source_pts is None:
                    ...
                    if dist_err < self.blind_spot_dist:
                        self.get_logger().info('사각지대 진입 -> 2차 Yaw 정렬로 전이')
                        state = DockingState.ALIGN_YAW
                    rate.sleep()
                    continue

                pts = self.latest_source_pts
                # 2. 물리적 안전 정지 거리 도달 여부
                if len(pts) > 0:
                    ...
                    if rear_to_wall_dist <= self.safety_stop_dist:
                        self.get_logger().info('후미 완전 밀착 도달 -> 2차 Yaw 정렬로 전이')
                        self.publish_stop()
                        state = DockingState.ALIGN_YAW
                        continue
                ...
                if match_cnt < 10:
                    icp_fail_count += 1
                    if icp_fail_count > 5:
                        self.get_logger().warn('ICP 매칭 장기 실패, 초기 위치 복원!')
                        current_transform = np.identity(3)
                        is_first_frame = True
                        icp_fail_count = 0
                    self.publish_stop()
                ...
                # 허용 오차 만족 시 2차 정렬로 전이
                if dist_err < self.dist_tolerance and abs(rel_y) < 0.015 and fitness > 0.4:
                    self.get_logger().info('ICP 오차 허용치 충족 -> 2차 Yaw 정렬로 전이')
                    self.publish_stop()
                    state = DockingState.ALIGN_YAW
                    continue
            ...
            # -------------------------------------------------------------
            # [STATE 2: ALIGN_YAW] 2차 정렬 (오도메트리 TF 기반 최종 Yaw 0.0)
            # -------------------------------------------------------------
            elif state == DockingState.ALIGN_YAW:
                try:
                    current_odom_yaw = self.get_current_odom_yaw()
                except Exception as e:
                    self.get_logger().warn(f'TF 룩업 실패 대기 중: {e}')
                    self.publish_stop()
                    rate.sleep()
                    continue
